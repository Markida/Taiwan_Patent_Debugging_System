...
whole folder must be transferred together; `SantoPatentOCR.exe` is only the
...
`packaging/SantoPatentOCR.Launcher.cs` builds the small Windows launcher. It
...
1. Run `SantoPatentOCR.exe --offline-self-test <report.json>` and require a
   `PASS` report.
2. Run `SantoPatentOCR.exe --gui-smoke-test` and require exit code 0.
