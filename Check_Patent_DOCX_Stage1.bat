...
"%PYTHON_EXE%" tools\inspect_patent_docx.py "%DOCX_PATH%"
if errorlevel 1 (
    echo.
    echo [ERROR] Stage 1 inspection failed.
    pause
    exit /b 1
)

for %%F in ("%DOCX_PATH%") do set "REPORT_STEM=%%~nF"
start "" "output\patent_review_stage1\%REPORT_STEM%_stage1_parse.html"

echo.
echo Report created under output\patent_review_stage1
pause
endlocal
